...
/*
How to determine stopID:
    Go to this page
    http://kovibusz.mefi.be/
    click on a stop, then choose a line, then
    in the opened URL there is the ID of the stop:
...
int main(int argc, char *argv[])
{
    ...
    // If the demo ends, the by reaching the DemoQuit::Quit slot we ask for to stop the application.
    DemoQuit quit;
    QObject::connect(&request, SIGNAL(QueryFinished()), &quit, SLOT(Quit()));
    // Sending the asynchronous request and entering the endless event handler loop.
    request.SendRequest("F01169");

    return a.exec();
}

/* Example for a short response:
  Server: nginx
